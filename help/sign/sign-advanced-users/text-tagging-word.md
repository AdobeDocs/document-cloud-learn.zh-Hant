---
title: 在 [!DNL Microsoft Word]中使用文字標籤
description: 瞭解如何在 [!DNL Microsoft Word]中新增Acrobat Sign文字標籤，以建立可重複使用的檔案範本
feature: Workflow, Sign
role: User, Admin
level: Intermediate
jira: KT-11163
thumbnail: KT-11163.jpg
exl-id: 48bc9cd0-019f-479b-9904-19e64354e2e7
TQID: https://experienceleague.adobe.com/D45aCOtiT4DmT90nryoI3LfrQDMCrwe8apPUNs6Py94
product_v2: id: b12c730b-5ddb-4a2d-ba42-da774988b909id: c1c5fb98-9105-44ed-9df1-9e04d062a784id: dc5cf79d-43c4-4731-bffa-1df5d7549cb1
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588
topic_v2: id: aa2f3246-cb95-4b30-8899-fdf7d73550cc
source-git-commit: d92345097c162b68b9d8405122534371c87c5f1a
workflow-type: tm+mt
source-wordcount: 81
ht-degree: 0%

---

# 在[!DNL Microsoft Word]中使用文字標籤

瞭解如何在[!DNL Microsoft Word]中新增Acrobat Sign文字標籤，以建立可重複使用的檔案範本。 文字標籤在檔案送出供簽名時，會自動在檔案上建立表單欄位，不需要手動放置欄位。 此程式可大幅減少您傳送出去供簽名的[!DNL Microsoft Word]份檔案進行修訂與變更所需的時間。

>[!VIDEO](https://video.tv.adobe.com/v/3409482?quality=12&learn=on&hidetitle=true)
