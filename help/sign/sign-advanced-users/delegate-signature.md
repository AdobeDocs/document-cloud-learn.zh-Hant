---
title: 使用委派者角色
description: 使用委派者角色將檔案傳送給中介者，中介者隨後可以將檔案路由以進行簽名
feature: Workflow, Sign
role: User
level: Intermediate
jira: KT-5322
thumbnail: 17366.jpg
exl-id: b9d7c6f4-a7fc-4cb8-a80d-91cc4c5b5298
TQID: https://experienceleague.adobe.com/wWrkYBvxXD3lXFLc8CabsBvBl-HNeOBEnc8KHF6q6YU
product_v2: id: b12c730b-5ddb-4a2d-ba42-da774988b909id: c1c5fb98-9105-44ed-9df1-9e04d062a784id: dc5cf79d-43c4-4731-bffa-1df5d7549cb1id: f002a92a-b99f-47a4-90c8-65e0e415bc7a
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588
source-git-commit: d92345097c162b68b9d8405122534371c87c5f1a
workflow-type: tm+mt
source-wordcount: 118
ht-degree: 3%

---

# 使用委派者角色

瞭解如何使用委派者角色將檔案傳送給中介者，中介者隨後可以將檔案路由以進行簽名。

>[!NOTE]
>
>委派者角色僅適用於企業級計畫。 您的Acrobat Sign管理員可能需要啟用&#x200B;**帳戶設定**&#x200B;的&#x200B;**傳送設定**&#x200B;區段中的&#x200B;**允許寄件者將部分收件者標示為委派者**&#x200B;設定，才能使用此功能。

>[!VIDEO](https://video.tv.adobe.com/v/343621?quality=12&learn=on&hidetitle=true)

>[!TIP]
>
>此影片是播放清單[使用者的進階工作](https://experienceleague.adobe.com/en/playlists/acrobat-sign-perform-advanced-tasks-business-users)的一部分，可在Experience League上免費取得！
