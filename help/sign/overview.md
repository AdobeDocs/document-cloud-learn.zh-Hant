---
title: Acrobat Sign教學課程
description: 教學課程、網路研討會和使用案例的集合，旨在讓初學者和管理員都能快速掌握Acrobat Sign
feature: Use Cases, Web Forms, Workflow, Sign, Payments, Report, Workspace, Deadline, Administration, Digital ID, Form, Integrations, Mobile, Skill Builder
role: User, Admin
level: Beginner, Intermediate, Experienced
jira: KT-6846
thumbnail: KT-6846.jpg
exl-id: 72762750-5764-4541-aedd-94230b56936b
TQID: https://experienceleague.adobe.com/cNKFwI8-wXh6JwurbNr5iiM7IpRpkwmdz-1zeZsZ3HU
product_v2: id: b12c730b-5ddb-4a2d-ba42-da774988b909id: c1c5fb98-9105-44ed-9df1-9e04d062a784id: dc5cf79d-43c4-4731-bffa-1df5d7549cb1
feature_v2: id: a0ba023f-45f7-46ff-bcd1-92ed8bb4d79did: a1028f9a-6dbc-4a4f-adf5-eb9f85a408a6id: a8a587a8-9b83-4ab3-a2c9-24c085c8c7ba
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: c66ffd68-0f65-42bb-aa23-b4020f12e0bd
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2: id: eddd9b14-83bd-4ff4-9072-54a4a484abb7
source-git-commit: d92345097c162b68b9d8405122534371c87c5f1a
workflow-type: tm+mt
source-wordcount: 193
ht-degree: 5%

---

# Acrobat Sign教學課程

Acrobat Sign為Adobe Document Cloud解決方案，可透過整合、API、進階驗證、其他管理功能等擴展以滿足您組織的需求。 您可以在這裡找到廣泛的學習體驗，這些體驗的設計目的是讓初學者和管理員都能快速掌握Acrobat Sign。

<div id="recs-overview-body-1"></div>
<div id="recs-overview-body-2"></div>
<div id="recs-overview-body-3"></div>
<div id="recs-overview-body-4"></div>
<div id="recs-overview-body-5"></div>
<div id="recs-overview-body-6"></div>

## 學習路徑

<table style="table-layout:fixed">
<tr>
  <td>
    <a href="sign-beginner-tutorials/beginner-users-overview.md">
      <img alt="快速入門" src="assets/gettingstartednew.png" />
    </a>
    <div>
    <a href="sign-beginner-tutorials/beginner-users-overview.md"><strong>快速入門</strong></a>
    </div>
    快速掌握如何傳送、簽署及追蹤檔案
    <br>
  </td>
  <td>
    <a href="sign-advanced-users/advanced-users-overview.md">
      <img alt="進階工作" src="assets/advanced-tasks.png" />
    </a>
    <div>
    <a href="sign-advanced-users/advanced-users-overview.md"><strong>進階工作</strong></a>
    </div>
    超越基本知識，完成特定工作與自動化
    <br>
  </td>
   <td>
    <a href="admin/intro-admin-overview.md">
      <img alt="管理員" src="assets/administer.png" />
    </a>
    <div>
    <a href="admin/intro-admin-overview.md"><strong>管理員</strong></a>
    </div>
    貴組織適用的基本到進階設定提示
    <br>
  </td>
   <td>
    <a href="digitalid/digitalid-overview.md">
      <img alt="數位ID" src="assets/identity.png" />
    </a>
    <div>
    <a href="digitalid/digitalid-overview.md"><strong>數位識別碼</strong></a>
    </div>
    瞭解如何在Acrobat Sign中使用全球的數位ID
    <br>
  </td>
</tr>
<tr>
    <td>
    <a href="integrations/integrations-overview.md">
      <img alt="整合" src="assets/integrations.png" />
    </a>
    <div>
    <a href="integrations/integrations-overview.md"><strong>整合</strong></a>
    </div>
    直接在另一個應用程式中新增Acrobat Sign
    <br>
    </td>
  <td>
    <a href="sign-usecase/expand-inspire-overview.md">
      <img alt="產業與部門" src="assets/industries.png" />
    </a>
    <div>
    <a href="sign-usecase/expand-inspire-overview.md"><strong>產業與部門</strong></a>
    </div>
    探索真實世界的產業和部門使用案例
    <br>
  </td>
  <td>
    <a href="develop/develop-overview.md">
      <img alt="開發" src="assets/develop.png" />
    </a>
    <div>
    <a href="develop/develop-overview.md"><strong>開發</strong></a>
    </div>
    在Acrobat Sign I/O上取得開發資源
    <br>
  </td>
  <td>
    <a href="deploy-overview.md">
      <img alt="部署" src="assets/deploy.png" />
    </a>
    <div>
    <a href="deploy-overview.md"><strong>部署</strong></a>
    </div>
    insight以及在組織內部署Acrobat Sign的最佳作法
    <br>
  </td>
</tr>
<tr>
  <td>
    <a href="mobile/mobile-overview.md">
      <img alt="行動裝置版本" src="assets/mobile.png" />
    </a>
    <div>
    <a href="mobile/mobile-overview.md"><strong>行動裝置</strong></a>
    </div>
    傳送、簽署及取得行動裝置上的即時更新
    <br>
  </td>
</tr>
</table>