---
title: 自訂命令和工具
description: 瞭解如何使用自訂命令和工具改善檔案工作流程效率
feature: Workflow, Workspace
role: User
level: Intermediate
jira: KT-9672
thumbnail: KT-9672.jpg
exl-id: 0cbe419b-d1fd-4737-9f79-0f99b4f38df3
TQID: https://experienceleague.adobe.com/XXdOIWxu7ov6vQOt6TYJzVHFFNu3QupwzKIky-9hPLg
product_v2: id: b12c730b-5ddb-4a2d-ba42-da774988b909
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2: id: b5a62a22-46f7-4f0d-b151-3fc640bef588
source-git-commit: d92345097c162b68b9d8405122534371c87c5f1a
workflow-type: tm+mt
source-wordcount: 72
ht-degree: 0%

---

# 自訂命令和工具

瞭解如何使用自訂命令和工具改善檔案工作流程效率。 然後，與同事共用您的新命令和工具，以幫助提高組織效率。 尚未針對[新的Acrobat體驗](../getting-started/new-experience.md)啟用此教學課程影片。

>[!NOTE]
>
>自訂命令僅適用於Acrobat Pro、Premium或Studio。

>[!VIDEO](https://video.tv.adobe.com/v/340545?quality=12&learn=on&hidetitle=true)
